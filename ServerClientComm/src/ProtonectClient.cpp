...
void displayMessage(){
    ...
    printf("\tStart/Stop -- Start/Stop grabbing images from all kinects\n");
    printf("\t&# Start/Stop -- Start/Stop grabbing images from individual kinect with "
           "last ip octate as #\n\n");
}
...
int main(int argc, char *argv[]){
    ...
    /* Create a second thread to receive the data sent by the peers.*/
    ...
    do
    {
        ...
        if (buffer[0] != 'q'){
            if (buffer[0] == '&'){								// Send to SINGLE MACHINE
                ...
                if (n < 0)
                    error("Sendto");
                     ...
                     (strncasecmp(buffer, "Start", 5) == 0)){	// Send to EVERYONE
